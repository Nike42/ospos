...
class Reports extends Secure_Controller
{
	...
	public function summary_employees($start_date, $end_date, $sale_type, $location_id = 'all')
	{
		...
		$tabular_data = array();
		$payment_type_summary_data = array();
		foreach($report_data as $row)
		{
			$tabular_data[] = $this->xss_clean(array(
			...
			$employee_payment_type_data = $model->prepareEmployeePaymentTypeSummary($inputs, $row['employee_id']);
			$employee_payment_type_data['employee_name'] = $row['employee'];
			$payment_type_summary_data[] = $this->xss_clean($employee_payment_type_data);
		}
		$paymentTypeSummaryDataColumns = $this->xss_clean($model->getPaymentTypeSummaryDataColumns());
		$data = array(
			...
			'data' => $tabular_data,
			'summary_data' => $summary,
			'payment_type_headers' => $paymentTypeSummaryDataColumns,
			'payment_type_summary_data' => $payment_type_summary_data
		);
	...
	public function specific_employee($start_date, $end_date, $employee_id, $sale_type)
	{
		...
		$payment_type_summary_data = $model->getPaymentTypeSummaryData($inputs);
		$data = array(
			...
			'details_data_rewards' => $details_data_rewards,
			'overall_summary_data' => $this->xss_clean($model->getSummaryData($inputs)),
			'payment_type_summary_data' => $this->xss_clean($payment_type_summary_data)
		);
