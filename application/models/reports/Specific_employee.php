...
class Specific_employee extends Report
{
	...
	public function getPaymentTypeSummaryData(array $inputs)
	{
		$where_sale_exits = $this->_get_where_sale_exits($inputs);
		$this->db->select('payment_type, IFNULL(SUM(payment_amount - cash_refund), 0) AS type_total');
	...
	// log_message('debug', 'employee_id->'.json_encode($this->db->get()->row()));
	// select payment_type, SUM(payment_amount - cash_refund) as total from ospos_sales_payments where employee_id=1 group by payment_type;
